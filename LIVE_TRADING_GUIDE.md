...
> The app does not require `MONGO_URL` or a literal env flag in the database layer. The live execution gate is enforced by the runtime state and the UI/API toggle, which stores a boolean `true`/`false` value.
