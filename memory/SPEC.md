...
## Auth
None — public read-only dashboard, no accounts or credentials.
...
`backend/lib/credentials.py` stores the CoinDCX key/secret and the live switch in Mongo
(`settings` collection, `_id="coindcx"`), falling back to `backend/.env` when the DB is
empty. The UI panel is `ApiKeysDialog` (header → **API Keys**): save keys, flip
PAPER ↔ LIVE, or delete keys. Secrets are only ever returned masked. Live orders require
`configured() and live_trading` — plus BOT ON and an armed strategy.
...
- CoinDCX 1h candles are hour-aligned (:00), so the "1h candle close at the :30 slot"
  is read as the last fully closed 1h candle at slot time.
