...
The backend and frontend verification suite is currently green:

- `cd backend && pytest -q`
- Result: `14 passed in 3.73s` on the strategy registry and engine regression set
- `cd frontend && npm run typecheck`
- Result: TypeScript validation passes without errors
...
- Mongo configuration accepts either a local URI or a cloud URI, with the order:
  `MONGO_URL`, `MONGO_URI`, `MONGO_CLOUD_URL`, `MONGO_URL_CLOUD`, then local fallback.
...
Scalping-main/
  backend/
    ...
    lib/
      ...
      credentials.py          Mongo-backed credential and live-mode state
