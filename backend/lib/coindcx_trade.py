...
async def signed_get(path: str, payload: dict[str, Any]) -> Any:
    raw, headers = _signed(payload)
    async with httpx.AsyncClient(base_url=BASE, timeout=httpx.Timeout(10, read=30)) as http:
        # CoinDCX signs a JSON body and sends it as a GET request; do not use the
        # POST helper here because the wallet endpoint is a GET and the wrong request
        # shape fails before auth is evaluated.
