...
def strategy_runtime_config(rule_set: str | None = None) -> dict[str, Any]:
    ...
    return get_strategy_runtime_config(rule_set)
...
def strategy_runtime_value(
    ...
    return strategy_runtime_config(rule_set).get(key, default)


def strategy_window_bounds(rule_set: str | None = None) -> tuple[dtime, dtime]:
    cfg = strategy_runtime_config(rule_set)
    start = cfg.get("WINDOW_START", dtime(5, 30))
    end = cfg.get("WINDOW_END", dtime(3, 40))
    return start, end


def in_window(moment: datetime | None = None, rule_set: str | None = None) -> bool:
    window_start, window_end = strategy_window_bounds(rule_set)
...
def order_window(timeframe: str, rule_set: str | None = None) -> int:
    cfg = strategy_runtime_config(rule_set)
    return cfg.get("ORDER_WINDOW", {}).get(timeframe, 300)
...
async def send_order_with_retry(
    ...
    for attempt in range(
        ...
        try:
            if market_entry:
                return await trade.place_market(
                    pair,
                    side,
                    quantity,
                    leverage,
            ...
            return await trade.place_limit(
                pair,
                side,
                price,
                quantity,
    ...
    async def _place(
        ...
        if trade.live_enabled():
            try:
                ...
                order = await send_order_with_retry(
                    market_entry,
                    pair,
                    side,
                    quantity,
