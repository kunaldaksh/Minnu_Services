...
def _env_value(name: str, default: str | int | float | bool, *, cast: type | None = None):
...
def get_str(name: str, default: str) -> str:
    return str(_env_value(name, default, cast=str))


def get_int(name: str, default: int) -> int:
    return int(_env_value(name, default, cast=int))


def get_float(name: str, default: float) -> float:
    return float(_env_value(name, default, cast=float))


def get_bool(name: str, default: bool) -> bool:
    return bool(_env_value(name, default, cast=bool))


def get_decimal(name: str, default: str | int | float | Decimal) -> Decimal:
    return Decimal(str(_env_value(name, default, cast=Decimal)))
...
RUNTIME_LIVE_MARGIN_UTILIZATION_LIMIT = get_decimal("RUNTIME_LIVE_MARGIN_UTILIZATION_LIMIT", "0.70")
