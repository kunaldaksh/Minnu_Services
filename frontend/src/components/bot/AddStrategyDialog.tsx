...
export default function AddStrategyDialog({
  ...
  const templateMap = Object.fromEntries(STRATEGY_TEMPLATES.map((template) => [template.rule_set, template])) as Record<RuleSet, StrategyTemplate>;
  ...
  const defaultRuleSet: RuleSet = STRATEGY_TEMPLATES[0]?.rule_set ?? "top4_5m_reversal_short";
